...
<template>
  <div class="p-4 flex flex-col gap-4">
    ...
    <form class="p-2">
      ...
      <UiInput v-model="credentials.username" label="Username" placeholder="huhu">
        <template #leftIcon>
          <span class="i-[mdi--mail] "></span>
        </template>

        <template #rightIcon>
          <button class="items-center text-center bg-slate-200"><span class="i-[mdi--mail] "></span></button>
        </template>
      </UiInput>
      <UiInput v-model="credentials.password" placeholder="huhu" />
      {{ credentials.username }}
      <button type="submit" @click.prevent="loginAsync">Username and Password</button>
    </form>
  </div>
</template>
...
const loginAsync = async () => {
  const { error } = await signIn('credentials', { username: credentials.username, password: credentials.password, redirect: false })
  if (error) {
